<template>
  <div>
    ...
    <h1 class="mb-8 text-3xl font-bold">
      <Link class="text-indigo-400 hover:text-indigo-600" href="/otpravki">Отправки</Link>
...
export default {
  ...
  methods: {
    update() {
      this.form.put(`/otpravki/${this.organization.id}`)
    },
    destroy() {
      if (confirm('Are you sure you want to delete this organization?')) {
        this.$inertia.delete(`/otpravki/${this.organization.id}`)
      }
    },
    restore() {
      if (confirm('Are you sure you want to restore this organization?')) {
        this.$inertia.put(`/otpravki/${this.organization.id}/restore`)
      }
    },
  },
}
